...
climate_values_10k <- terra::as.data.frame(climate_10k)
...
## Working with MIS
Sometimes we want to combine multiple time steps to represent the background
(e.g. when considering a given MIS). `pastclim` has a function to get a vector
...
We now cycle over those steps. First extract the climate into a list:
```{r}
...
If we are interested in a single time step, we can simple generate the 
