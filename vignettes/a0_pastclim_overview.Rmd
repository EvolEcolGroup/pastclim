...
in Nigeria), La Riera (a Late Paleolithic coastal site on Spain), Chalki (a Mesolithic site on a 
...
library(ggplot2)
ggplot(data=locations_ts, aes(x=time, y=bio01, group=name)) +
...
interpolate from nearest neighbours. So, to get data for that island, we would have
to change slighlty the coordinates to make sure it falls within a cell with climatic
information.

Pretty labels for environemntal variables can be generated with `var_labels`:
```{r, warning=TRUE, fig.width=4, fig.height=3}
library(ggplot2)
ggplot(data=locations_ts, aes(x=time, y=bio01, group=name)) +
  geom_line(aes(col=name))+
