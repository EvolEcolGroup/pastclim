...
```{r}
location_slice (x= locations[,c("longitude", "latitude")],
...
```{r}
locations_ts <- location_series(x= locations[,c("longitude", "latitude")],
...
```{r}
climate_20k <- region_slice(time_bp = -20000,
                                      c("bio01","bio10","bio12"),
                                      dataset="Example")
```
...
```{r, fig.width=6, fig.height=5}
climate_20k <- region_slice(-20000,
                                      c("bio01","bio10","biome"),
                                      dataset = "Example")
...
```{r, fig.width=4, fig.height=4}
locations_10k <-location_slice (x= locations[,c("longitude", "latitude")],
           time_bp = locations$time_bp, bio_variables=c("bio01","bio12"),
           dataset="Example")
locations_10k <- locations_10k[locations_10k$time_bp_slice == -10000,]
locations_10k_climate<- location_slice (x= locations_10k[,c("longitude", "latitude")],
...
for (this_step in mis_time_steps) {
  this_step_climate <-
    region_slice(time_bp = this_step ,
...
climate_20k <- region_slice(time_bp = -20000,
                            ...
                            dataset = "Example")
this_sample<-terra::spatSample(climate_20k, 100, na.rm = TRUE,
                               cells = TRUE,xy = TRUE)
head(this_sample)
```
...
```{r}
additional_var <- location_slice (x = this_sample$cell,
           time_bp = rep(-20000, nrow(this_sample)), 
           bio_variables = c("bio12"),
           dataset = "Example")
head(additional_var)
