...
We create a rater from a list of the files:
```{r}
...
Note that this reconstructions include the ocean and the icesheets, and it would
be much better to remove them as they are not needed for most ecological work (and
it allows for smaller files).

# Making the data available to others

In order to make data available to others, you need to:

1) Put your files in a freely available repository.

2) Update the table used by `pastclim` to
store information about possible datasets. This table is found in
...
This includes the followign fields:
...
3) Run the script "./inst/rawdata_scripts/creat_files_by_dataset.R" to store
