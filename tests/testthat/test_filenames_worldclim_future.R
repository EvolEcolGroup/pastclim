...
test_that("filenames_worldclim_future are correct", {
  ...
  target_datasets <- target_datasets[!(grepl("FIO-ESM-2-0_ssp370",x = target_datasets) |
  ...
  for (i in seq_len(length(target_datasets))){
    bio_files <- filenames_worldclim_future(dataset = target_datasets[i], 
                                         bio_var = sample(variables, 1))
    expect_true(url_is_valid(bio_files[sample(1:4,1)]))
  }
  # monthly variables
  variables <- c(paste0("temperature_min_",sprintf("%02d", 1:12)),
