...
#' than `land_mask_high_res`, as defined by `expand_xy`. After applying
...
#' @param d a numeric vector of length 2, specifying the parameters for the Gaussian
...
#' @param expand_xy a numeric vector of length 2, specifying the number of units to
...
downscale_ice_mask <- function(ice_mask_low_res, land_mask_high_res, d = c(0.5, 3),
                          expand_xy = c(5,5)) {
  ice_mask_low_res_bin <- make_binary_mask(ice_mask_low_res)
  # create a margin around the land mask to avoid edge effects when using the Gaussian filter
  ice_mask_high_res <- resample(ice_mask_low_res_bin, 
                                extend(land_mask_high_res, terra::ext(as.vector(terra::ext(land_mask_high_res))+
