...
download_etopo_subset <- function(rast_template, ...) {
  ...
  if (!all.equal(terra::res(rast_template)[1], terra::res(rast_template)[2])) {
    stop(
      "rast_template should have identical vertical ",
      "and horizontal resolution"
    )
  }

  relief_bathy <- marmap::getNOAA.bathy(
    lon1 = terra::ext(rast_template)[1],
  ...
  # ideally check if there are any differences (due to rounding problems or
  # corners), and then resample the relief to match the template exactly
  if (!all(
    terra::ext(relief_rast) == terra::ext(rast_template),
