...
#' given for `size`), or for different locations for each time step (if `size`
...
#' [`terra::SpatRaster`]s in the [`terra::SpatRasterDataset`] returned by
#' [region_series()].
#'
#' @param x a [`terra::SpatRasterDataset`] returned by [region_series()]
#' @param size number of points sampled. A single value is used to sample the
...
sample_region_series <- function(x, size, method = "random",
                                 replace = FALSE, na.rm = TRUE) { # nolint
  if (!is_region_series(x)) {
    stop("x is not a valid object generated by region_series")
  }

  if (length(size) == 1) {
    sample_rs_fixed(x, size,
      method = method, replace = replace,
      na.rm = na.rm
    )
  } else {
    sample_rs_variable(x, size,
      method = method, replace = replace,
      na.rm = na.rm
    )
  }
...
sample_rs_fixed <- function(x, size, method = "random", replace = FALSE,
                            na.rm = TRUE) { # nolint
  ...
  sampled_values <- sampled_cells[
    rep(seq_len(nrow(sampled_cells)),
...
sample_rs_variable <- function(x, size, method = "random", replace = FALSE,
                               na.rm = TRUE) { # nolint
  if (sum(size) == 0) {
  ...
  for (i in seq_len(length(size))) {
    if (size[i] > 0) {
      ...
      # after removing NA. The default exp=5 is not sufficient if size is very
      # small.
      if ((size[i] * 5) < 1000) {
