####################### ' Set vrt metadata ' ' This function sets the metadata
# information to a vrt file. It creates some dataset ' wide metadata, as well as
# band specific descriptions and times. ' @param vrt_path path to the XML file
# defining the vrt dataset ' @param description a string with the description of
# the variable in this dataset ' @param time_vector a vector of descriptions
# (same length as the number of bands) ' @param time_bp boolean defining whether
# time is in BP or (if FALSE) CE ' @returns TRUE if the file was updated
# correctly, FALSE if the update failed ' ' @keywords internal # Edit the vrt
# metadata
vrt_set_meta <- function(vrt_path, description, time_vector, time_bp = TRUE) {
  # read vrt file
  x <- xml2::read_xml(vrt_path)
  # TODO ideally we should check that we don't already have metadata to avoid
  # writing it twice check that we don't alreayd have metadata information
  # needed for pastclim
  has_time_node <-
  ...
  xml2::xml_add_child(metadata_node, "MDI",
    key = "pastclim_time_bp",
    tolower(as.character(time_bp))
  )
  ...
  if (length(band_nodes) != length(time_vector)) {
    warning(
      "the vrt ", vrt_path,
      " has a different number of bands from the ",
  ...
  for (i_node in seq_len(length(band_nodes))) {
    ...
    metadata_node <- xml2::xml_find_first(band_nodes[i_node],
      xpath = "Metadata"
    )
    xml2::xml_add_child(metadata_node, "MDI",
      key = "time",
