...
#'
#' This function sets the path where climate reconstructions will be stored. This
...
set_data_path<-function(path_to_nc=NULL){
  ...
  if (is.null(path_to_nc)){
    cat(tools::R_user_dir("pastclim","data"), "\n", 
        file = file.path(tools::R_user_dir("pastclim","config"),
                         "pastclim_data.txt"))
...
#'
#' This function returns the path where climate reconstructions will be stored. This
#' information is stored in a file names "pastclim_data.txt", which is found
#' in the directory returned by `tools::R_user_dir("pastclim","config")` (i.e.
#' the default configuration directory for the package as set in R >= 4.0).
#' 
...
get_data_path<-function(){
  ...
  if (!is.null(getOption("pastclim.data_path"))){
    ...
    if (!file.exists(file.path(tools::R_user_dir("pastclim","config"),
                               "pastclim_data.txt"))){
      set_data_path()
    }
    path_to_nc =  utils::read.table(file.path(tools::R_user_dir("pastclim","config"),
                                         "pastclim_data.txt"))[1,1]
    return(path_to_nc)
  }
}
...
copy_example_data <-function() {
  if (!file.exists(file.path(get_data_path(),"example_climate_nc"))){
    file.copy(from = system.file("/extdata/example_climate.nc", 
                                 package="pastclim"),
