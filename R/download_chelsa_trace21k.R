...
#'
#' This function downloads annual and monthly variables from the CHELSA trace
#' v1.0 dataset.
#'
#' As this dataset is huge, we should not download all files in most situations.
#' For this reason, time_bp has to be set for downloading (but it is allowed for
#' virtual datasets)
#'
...
#' @param time_bp the time steps for which the dataset should be built (NULL for
#'   all time steps)
...
download_chelsa_trace21k <- function(dataset, bio_var, filename = NULL,
                                     time_bp = NULL) {
  ...
  if (!all(time_bp %in% avail_time_bp)) {
    stop(
      "some values in time_bp are not valid; only steps from 0 to ",
      "-22000 in steps of -100 are valid"
    )
  }
  ...
  download_url <- filenames_chelsa_trace21k(
    dataset = dataset,
    bio_var = bio_var
  )
  ...
  # hacks to deal with missing file in temperature_min_07 and temperature_max_07
