...
beyer_temp <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Beyer2020",
...
beyer_prec <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Beyer2020",
...
beyer_pastclim_biovar <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Beyer2020",
...
for (i in names(beyer_biovar)) {
  cor_all[i] <- cor.test(
    as.matrix(beyer_biovar[[i]]),
    as.matrix(beyer_pastclim_biovar[[i]])
  )$estimate
}
cor_all
...
cor.test(
  as.matrix(beyer_pastclim_biovar$bio16),
  as.matrix(beyer_pastclim_biovar$bio18)
)$estimate
