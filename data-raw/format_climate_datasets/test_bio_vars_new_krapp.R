...
pastclim::set_data_path(path_to_nc = "~/project_temp/mario_data/", copy_example = FALSE)


# krapp_new <- terra::rast(dir("./"))


krapp_temp <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Krapp2021",
  bio_variables = paste0("temperature_", sprintf("%02d", 1:12))
)
krapp_prec <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Krapp2021",
...
krapp_pastclim_biovar <- pastclim::region_slice(
  time_bp = 0,
  dataset = "Krapp2021",
...
for (i in names(krapp_biovar)) {
  cor_all[i] <- cor.test(
    as.matrix(krapp_biovar[[i]]),
    as.matrix(krapp_pastclim_biovar[[i]])
  )$estimate
}
cor_all

krapp_pastclim_biovar[101, 150]
...
cor.test(
  as.matrix(krapp_pastclim_biovar$bio16),
  as.matrix(krapp_pastclim_biovar$bio18)
)$estimate
