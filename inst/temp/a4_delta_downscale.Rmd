...
coastlines following sea level change. We will use an upscaled version of the
ETOPO1 dataset (upscaled to 5 min resolution to keep the file smaller):
...
ext(topo_rast) == ext(high_res_obs)
ncol(topo_rast) == ncol(high_res_obs)
nrow(topo_rast) == nrow(high_res_obs)
```
...
```{r, fig.width=6, fig.height=5}
high_res_mask <- pastclim:::make_land_mask(topo_rast = topo_rast, 
...
```{r}
delta_rast<-pastclim:::delta_compute(x=model_rast, ref_time = 0, obs = high_res_obs)
model_downscaled <- pastclim:::delta_downscale (x = model_rast, delta_rast = delta_rast,
