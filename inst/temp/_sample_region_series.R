...
#' \code{terra::spatRaster}s in the \code{terra::spatRasterCollection} returned
#' by \code{region_series}.
#'
#' @param x a \code{terra::spatRasterCollection} returned
#' by \code{region_series}
...
sample_rs_variable<-function(x, size, method="random", replace=FALSE, na.rm=TRUE)
{
  if (length(size)!=length(x)){
    stop("size should be the same length as the number of variables in x")
  }
  ...
  for (i in seq_len(length(size))){
    ...
    # write into output
    climate_list[[as.character(t_steps[i])]]<-values
  }
  # combine them into a single matrix
  sampled_climate <- do.call(rbind, climate_list)
